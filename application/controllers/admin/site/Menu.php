...
/**
 * Controle para busca e cadastro de Menus no Site
 *
 * @author Thiago Moura
 */
class Menu  extends MY_Controller{
    ...
    public function __construct(){
        $config['control_url'] = 'admin/site/menu';
    ...
    public function busca($data = array()){
        ...
        if(!isset($data['menus'])){
            $data['menus'] = $this->menu_model->selecionar('*','sistema = 0','grupo ASC, ordem ASC');
        }
    ...
    private function _get_options_grupo(){
        $grupos = array();
        foreach($this->menu_model->selecionar_distinto('grupo','sistema = 0','grupo ASC') as $menu){
